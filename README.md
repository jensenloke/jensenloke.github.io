...
1. Add a new HTML file inside its track directory (`engineering/`, `reviews/`,
   `second-brain/`, or `local-models/`).
2. Add an entry to that track's `index.html` (newest first).
3. Add an entry to `essays/index.html`.
...
1. Copy `papers/template.html` to `essays/<paper-slug>.html` and fill it in.
2. Add an entry to `essays/index.html` (newest first, tagged *Paper*).
3. For second-brain research, also add the entry to `second-brain/index.html`.
